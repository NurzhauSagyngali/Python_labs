...
class PaintApp:
    ...
    def handle_events(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:
                    if self.brush.mode in ["rect", "circle", "right_triangle", "equilateral_triangle", "rhombus"] and self.brush.start_pos:
                ...
                if key in self.colors:
                    ...
                    self.brush.set_mode("right_triangle")
                elif event.key == pygame.K_y:
                    self.brush.set_mode("equilateral_triangle")
                elif event.key == pygame.K_h:
                    self.brush.set_mode("rhombus")
        return True

    def instructions(self):
        instructions = [
            "Z: RECT | O: CIRCLE | T: RIGHT TRI | Y: EQUIL. TRI | H: RHOMBUS",
            "P: BRUSH | E: ERASER | C: CLEAR SCREEN",
    ...
    def run(self):
        ...
        while running:
            ...
            if self.brush.drawing and self.brush.mode in ["rect", "circle", "right_triangle", "equilateral_triangle", "rhombus"] and self.preview:
